// Global declarations (make sure these are defined elsewhere as needed)
#define MAX_SERIAL_BUFFER 500

byte incomingByte = 0;
byte incomingdata[500];
int data_index = 0;
byte databuffersize = 4096;
int databufferposition = 0;
//char InputOutputsString[20][20];
int SerialCommand;
int EEPROMStorage;

// Assume these global arrays and variables are declared and sized appropriately:
// OutputChannel[], OutputType[], OutputRelay[], OutputMinValue[], OutputMaxValue[], 
// OutputDefaultValue[], OutputValueActual[], MQTT_Name[][], MQTT_Output[][],
// SwitchCounter, OutputCounter, InOutMatrix[][], InOutMatrixCouter[],
// MQTT_Input[][], MQTT_Output[][], etc.

// Also assume DynamicVariable, DYNAMIC_STRING, DYNAMIC_BYTE, DYNAMIC_INT, DYNAMIC_IP,
// DYNAMIC_PASSWORD and ProcessVariablesByPriority are defined elsewhere.

// ----- New: Templated ProcessVariablesByPriority -----
// This allows passing a lambda with captures (such as by-reference EP)
            ...
            case 2: // Generic
                ...
                switch (Type02) {
                    ...
                    case 2: //only type 2 can contain data
                        ...
                        break; 
                    case 3: break; // should not be changable externally
                    case 4: break; // should not be changable externally
                    case 5: break; // should not be changable externally
                    default: 
                        snprintf(msgmqtt, sizeof(msgmqtt), "%s%s", MQTTName, "/GENERIC");
